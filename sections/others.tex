...
\begin{algorithm}
	...
	\end{algorithmic}
\end{algorithm}
Arrival of a new vertex creates at most one short augmenting path on the assumption that no existed before, and removes it, maintaining the invariant that there are no short augmenting paths.
Therefore, the algorithm that greedily removes short augmenting paths as new vertices arrive has a competitiveness of $\frac{d}{d+1}$