...
\begin{proof}
	Let $m=\ceil{f(n)}+1$.
	For $\mathrm{ALG}$ construct the adversary graph in a manner similar to the one above, with $m$ isolated $I^\star$ (from 3.2) adversarial subgraphs on $\frac{n}{m}$ vertices, with online vertices arriving to each subgraph in the same adversarial order while all subgraphs only have one online vertex in the delay window, implying that they receive their $k$-th online vertex before any receives it's $k+1$'st online vertex. Vertex labels are uniform random, as is the order in which that first $m$ vertices arrive.

	The behavior of the algorithm, isolated to each subgraph, creates a family of some Semi-Online algorithms. This is evident as an algorithm for Semi-Online Matching can pretend that there are $m-1$ other subgraphs and this specific delay window exists. By solving this false instance, a true Semi-Online instance is also solved. To achive a competitiveness ratio above $1-\frac{1}{e}$, at least one of the subgraphs algorithms would have achieved this same higher ratio on $I^\star$, which we know is impossible from section 3.2 (and further from \cite{karp}), where it is proven that all algorithms have a performance upper bound of $1-\frac{1}{e}$ on $I^\star$ on the condition that vertex labels are random.
...
\begin{proof}
	...
	As $v$ arrives, if no short path is created then the algorithm does nothing and the invariant is maintained. If a shortest short $M$-augmenting path $P$ is created, the current matching is $M$, and $M$ augmented by $P$ is $M'$. Let $Q$ be any $M'$-augmenting path. We will prove that there exist $M$-augmenting paths $X,Y$ such that
	\begin{itemize}
		\item $X,Y$ are edge-disjoint
		\item $X\cup Y\subseteq P\cup Q$
		\item the sets of endpoints for $P,Q$ and $X,Y$ are identical
	\end{itemize}
	since all vertices in $P$ are matched in $M'$, trivially $Q$ has a separate set of endpoints from $P$. In a symmetric difference graph $H=(U,V,P\triangle Q)$ Four vertices, the endpoints of $P,Q$, have a degree of $1$ and all others have a degree of $2$ since otherwise that vertex would be internal to $Q$ and on $P$, implying the existence of an edge $e$ in $P\backslash M$ connected to it, implying that it is in $M'$, and in turn in $Q$, leading to a contradiction as this edge is also in $P\triangle Q$.\\
	...
	First, the endpoints of $Q$ must be unmatched in $M$ as they are not on $P$. notice that an edge in $Q\backslash P$ is in $M$ iff it is in $M'$. Let vertex $v$ have degree $2$ in $H$. If both of it's incident edges in $H$ are from $P$ or $Q$ then the statement holds. Else, $v$ in on $P$ and $Q$, and isn't and endpoint of $Q$. Similar to before, there must be an edge $e$ incident to $v$ that is $P\backslash M$ and $Q\cap M'$, implying that one of the incident edges from $H$, the one in $P\triangle Q$ is in $P\cap M$ and the other edge is in $Q\backslash(M\triangle P)=Q\backslash M$, making $X,Y$ augmenting paths.\\
	
	WLOG, suppose that the arriving $v$ is an endpoint of $X$. Trivially, it is not shorter than $P$ since they are both $M$-augmenting paths and the shorter on is used by the algorithm to augment $M$ to $M'$. Further, $v$ isn't an endpoint of $Y$, which is also an $M$-augmenting path. By induction hypothesis $Y$ is not short. $X\cup Y\subseteq P\cup Q$ implies $|X|+|Y|\leq|P|+|Q|$, implying $|X|+|Y|\leq|X|+|Q|$ resulting in $|Y|\leq|Q|$, meaning that $Q$ must not be short.
\end{proof}