\begin{algorithm}
	...
	\end{algorithmic}
\end{algorithm}
Despite this algorithm seeming promising, it is only marginally more effective than a naive deterministic approach. We will show that in this model no deterministic algorithm has asymptotic competitiveness greater then $0.5$
\begin{theorem}
...
A randomized version of the previous Effective Degree algorithm is

\begin{algorithm}
	\caption{Randomized Effective Degree - RED}
	\label{alg:red}
	\begin{algorithmic}
		\item on Initialization: for each offline vertex $u_i$ let $d_{u_i}=\deg(u_i)$
		\item on Arrival of vertex $v$: $N$ are unmatched offline neighbors of $v$. If $N\neq\emptyset$, match $v$ to random $u$ with $d_u=\underset{u'\in N}{\mathrm{min}}\ d_{u'}$, $\underset{u\in N}{\forall}d_u-=1$
	\end{algorithmic}
\end{algorithm}

\begin{lemma}
\label{deg-diff-degree}
If all vertices have different degrees and $G$ has a perfect matching then RED finds a perfect matching
\end{lemma}

\begin{proof}
	Let $G_n=(V,U,E)$ be a $n\times n$ bipartite graph with with all vertices in $U$ having different degrees. Since $G$ has a perfect matching, implying that the degrees of $u\in U$ are $1,\dots,n$. Let $u_k:=u\in U|deg(u)=k$\\
	To force the algorithm to leave $n_k$ behind, $k$ online vertices need to arrive, each connected to a different $u'_i\in\{u_i|i<k\}$. The problem with this is that the size of $U'$ is $k-1$, making such a thing impossible
\end{proof}
...
RGA (\autoref{alg:RGA}) having an expected matching size of $\frac{n}{2}+O(\log n)$ according to \cite{karp}, it is expected to find a matching of size $n(1-\frac{1}{e})$

\begin{lemma}
	On $G_{\deg}$ from \autoref{fig:deg-det-comp} \autoref{alg:red} has an expected performance of no less than $\approx 0.75$
\end{lemma}
\begin{proof}
	...
	In the second phase $k$ vertices arrive connected to incrementally smaller parts of the upper $k$ offline vertices of which we expect half to be available for matching. Using an argument symmetrical to the one in \autoref{deg-diff-degree} we know that the matching here is the best possible, however we expect only half of the lower offline vertices to be matchable\\
	In total - all online vertices in the first phase have been matched and an expected half of the vertices in the second phase has been matched, combined with the fact that both phases have half of all online vertices implying that an expected performance of $0.75$
\end{proof}

%Note here that we have conducted Monte Carlo experiments  with this exact algorithm and graph for $k$ up to $50$ and in over 1 million simulations the algorithm found a matching of average size $n-2$ suggesting that the actual performance is much higher here - create appendix if this is kept

Despite these seeming advantages, this algorithm doesn't have a competitiveness above $0.5$
\begin{theorem}
	Any algorithm that prioritizes vertices based on degree (meaning that they either match high or low degree vertices first) has an asymptotic competitiveness of no more than $0.5$
\end{theorem}
\begin{figure}[H]
	\centering
	\input{figures/degree/priority.tikz}
\end{figure}
\begin{proof}
	The case in which higher degree vertices are prioritized for matching is trivial as the counterexample is the same $I^\star$ from Section 3.2 (Ranking Optimality) and is added here for completion. In the case that the algorithm prioritized lower degree vertices like RED we construct the graph as follows: $k$ upper offline vertices $u_1,dots,u_k$ with degree $k$ and $k$ lower offline vertices $u_{k+1},dots,u_{2k}$ with degree $2$. Online vertices arrive in two stages: first, $k$ vertices $v_1,\dots,v_k$ connected to the upper half, where $v_i$ is also connected to $u_k+i$. These will all, except possibly the last two, be matched to $u_k+i$ as only then the effective degrees between upper and lower half equalize. This in then makes the next $k$ online vertices unmatchable as $u_{k+i}$ is only connected to $v_{k+i}$
\end{proof}
We note that the graph for low-priority algorithms was devised by \cite{obmRaRank} to fool their MPD algorithm