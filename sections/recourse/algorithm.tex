we present the algorithm and proof from \cite{obmRecourse}
\begin{algorithm}
	\caption{SAP}
	\label{alg:sap}
	\begin{algorithmic}
...
\begin{proof}
	...
	\end{enumerate}
	we conclude that if $M_t$ isn't maximum at time $t+1$ then there must be an augmenting path $P$ that didn't exist at time $t$, meaning that the newly arriving $v$ must be it's starting point implying $P=(v,v_1,\dots,v_k)$. If switching $P$ creates a new augmenting path $R$ it must be joined to the switched path with an unmatched edge adjacent to $v_i$ and another unmatched edge, WLOG from $v_0$ to some $u$. Notice that, before $P$ was swapped, $(w,\dots,v_k)$ was an augmenting path as well, contradicting the assumption. Do note that $u$ may be connected like $w$ (denoted $u'$) an is then defined as the endpoint whose separation point from $P$ is closer to $v$, but a symmetrical proof by contradiction still applies.
\end{proof}
\begin{figure}
	\centering
...
We will now prove that the balanced server flow is unique (regarding $\alpha$-values) using an auxiliary lemma
...
\begin{lemma}
	...
	The server flow in \autoref{balanced-sf-iff} is unique in terms of $\alpha$ if it exists
\end{lemma}
\begin{proof}
	Using \autoref{balanced-sf-aux} we can argue that the balanced server flow is the optimum of the convex program in the proof of \autoref{balanced-sf-iff}.\\
...
\begin{lemma}
	...
	$\forall u\in U:\Delta\alpha(u)\geq 0$
\end{lemma}
\begin{proof}
	...
	The server flow being balanced at $t$ implies there is no edges from $V^\Delta$ to $U^-$ at time $t$.\\
	Further, there are no active edges from $V^\Delta$ to $U^+$ at time $t+1$\\
...
\begin{proof}
	...
	Let $U^-=U\backslash U^+$ be complement of $U^+$\\
...
\begin{lemma}
	Let $\overline{\alpha}=\underset{u\in U}{\max}\alpha(u)$ be maximal necessity, $\overline{U}=\set{u\in U|\alpha(u)=\overline{\alpha}}$ maximally necessary servers, $K'=\set{v\in V|A(v)\cap\overline{U}\neq\emptyset}$ active neighbors, then $N(K')=\overline{U}$, $|K'|=\overline{\alpha}|\overline{U}|$
\end{lemma}
\begin{proof}
...
\end{lemma}
\begin{proof}
	Definition of server flow implies, for $K\subseteq V$ $|K|\leq\sum\limits_{u\in N(K)}\alpha(u)\leq\overline{\alpha}|N(K)|$ implying $|K|\leq\overline{\alpha}|N(K)|$.\\
	Let $K'$ be defined as in above lemma. Then $\overline{\alpha}=\frac{|K'|}{N(K')}\leq\underset{\emptyset\subset K\subseteq V}{\max}\frac{|K|}{|N(K)|}\leq\overline{\alpha}$\\
...
\begin{lemma}[Expansion]
	...
	Assume $\alpha_M(u)=1-\epsilon$ for some $u\in U,\epsilon>0$. There is an active augmenting tail for $u$ of length at most $\frac{2}{\epsilon}\ln(|V_M|)$
\end{lemma}
\begin{proof}
