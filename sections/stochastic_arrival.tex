...
Since the graph is now random, competitiveness has to be redefined as $M^\star$ is also random. In this context, we define the competitiveness ratio as $\frac{\E[\mathrm{ALG}]}{\E[\mathrm{OPT}]}$. We also note that \cite{obmStochastic}, whose analysis we follow, didn't redefine competitiveness, though their analysis is consistent with this definition.

We will present the proof of two theorems in this chapter:

\begin{theorem}
	No algorithm has a competitiveness of $1-\epsilon$ for some $\epsilon>0$ in the iid model
\end{theorem}
meaning that there is no perfect algorithm, and
\begin{theorem}
	\label{thm:sa-alg}
	The stochastic arrival model is strictly easier than the Semi-Online model since there is an algorithm, TSM, with competitiveness $\approx 0.670$
\end{theorem}
