In this section we will prove \autoref{thm:rank-ra-comp} following an approach by \cite{obmRaRank}\\
...
\end{enumerate}
Do note that the last step is computational and does not prove an exact competitiveness ratio. Indeed, this approach merely shows that the ratio is at least $0.696$, and other analysis show it can be no larger than $0.727$ according to \cite{rank-ra-upper-bound}. The exact competitiveness ratio is currently unknown.
...
\begin{lemma}[Monotonicity]
	$\forall \sigma\in\Perm{U},\pi\in\Perm{V},u_{\mathrm{new}},u_{\mathrm{old}},u\in U,v\in V$ such that $u_{\mathrm{new}}<u_{\mathrm{old}},u<u_{\mathrm{old}}$,
	Let $\sigma'$ be $\sigma$ where $u_{\mathrm{new}}$ and $u_{\mathrm{old}}$ swap places.
	\begin{equation}
		\label{ra-rank-eq2}
		\sum\limits_{1\leq u'\leq u + \mathrm{geq}(u,u_{\mathrm{new}})}x(\sigma',\pi,u',v)\geq\sum\limits_{1\leq u'\leq u}x(\sigma,\pi,u',v)
	\end{equation}
\end{lemma}
\begin{proof}
...
\begin{equation}
	\label{ra-rank-eq3}
	\sum\limits_{1\leq v'\leq v + \mathrm{geq}(v,v_{\mathrm{new}})}x(\sigma,\pi',u',v)\geq\sum\limits_{1\leq v'\leq v}x(\sigma,\pi,u,v')
\end{equation}
...
\begin{lemma}
	The competitiveness ratio of Ranking in the Random Arrival model is lower bounded by $\inf\limits_{n\in N}\inf\limits_{k}\mathrm{expLP}(n,k)$ for any infinite subset of natural numbers $N$
\end{lemma}
\begin{proof}
...
Let $x_2(u,v,q)$ be the probability over random $\sigma,\pi\in\Perm{U}\times\Perm{V}$ that offline agent of rank $u$ is matched to an online agent at time $v$ by Ranking and that the online agent arriving at time $q$ will be matched to the offline agent at rank $u$ in $M^\star$
...
\sum\limits_{1\leq u'\leq u}x_i(u',v,p) & \text{else}
\end{cases}\]

Now we derive the new inequalities for $\mathrm{PolyLP}(n)$
multiplying both sides of \autoref{ra-rank-eq1} by $\mathrm{eq}(\sigma(u)m\pi(v))$ gives

\[\sum\limits_{1\leq u'\leq u}\mathrm{eq}(\sigma(u),\pi(v))x(\sigma,\pi,u',v)+\sum\limits_{1\leq v'\leq v}\mathrm{eq}(\sigma(u),\pi(v))x(\sigma,\pi,u,v')\geq\mathrm{eq}(\sigma(u),\pi(v))\]

and taking expectation over random $\sigma,\pi$ results in

\begin{equation}
	\label{ra-rank-eq4}
	y_1(u,v,u)+y_2(v-1,u,v)\geq \frac{k}{n^2}
\end{equation}
...
Notice that $\sigma'$ is uniformly random, $\sigma(p)=\pi(v)$ iff $\sigma''(p')=\pi(v)$ where $\sigma''$ which $\sigma$ with $u_{\mathrm{new}},n$ swapped and
...
\begin{equation}
	\label{ra-rank-eq5}
	y_1(u+\mathrm{geq}(u,u_{\mathrm{new}}),v,p')\geq y_1(u,v,p)
\end{equation}
...
\begin{equation}
	\label{ra-rank-eq6}
	y_2(v+\mathrm{geq}(v,v_{\mathrm{new}}),u,q')\geq y_1(u,v,q)
\end{equation}
...
\begin{itemize}
	...
	\item and several other concerning certain constraints that become trivial for variables in certain ranges, which we will not recount
\end{itemize}
which finally results in
\begin{align*}
	\text{Minimize} & \frac{1}{n}\sum\limits_{u,v,p\in[n]}x(u,v,p)\\
	\text{subject to} & y(u,v,u)+y(v-1u,v)\geq \frac{1}{n}\\
	& y(u+1,v,p+1)\geq y(u,v,p) & \forall p\leq u <n\\
	& y(u,v,p)=y(u,v,u+1) & \forall u<p\\
	& y(u+1,v,p)\geq y(u,v,u+1) & \forall p\leq u < n\\
	& \sum\limits_p x(u,v,p)= \sum\limits_p x(v,u,p)\\
	& y(u,v,p)=\sum\limits_{1\leq u'\leq u}x(u',v,p)
\end{align*}
...
\end{lemma}
\begin{proof}
Take any integer $m>0$. For any instance of size $n=km,k\in\mathbb{Z}$. We will prove that the competitiveness ratio of Ranking is lower-bounded by $\mathrm{PolyLP'}(m)$. We know that $\inf\limits_d\mathrm{PolyLP}(md)$ is a lower bound on the desired competitiveness ratio. We will take an optimal solution $(x,y)$ of $\mathrm{PolyLP}(md)$ and project it to a feasible solution of $\mathrm{PolyLP'}(m)$ as follows
	\begin{definition}
		...
		For any $u',v',p'\in[m]$ define $x'(u',v',p')=\frac{1}{d}\sum\limits_{(u,v,p)\in f(u',v',p')}x(u,v,p)$,$y'(u',v',p')\allowbreak=\frac{1}{d}\sum\limits_{(v,p)\in f(v',p')}y(u'd,v,p)$
	\end{definition}
	...
	\begin{align*}
		y'(u'+1,v',p'+1) &=\frac{1}{d}\sum\limits_{(v,p)\in f(v',p')}y(u'd+d,v,p+d)\\ 
		                 &\geq \frac{1}{d}\sum\limits_{(v,p)\in f(v',p')} y(u'd,v,p)\\
						 &=y'(u',v',p')
	\end{align*}
	where the inequality is the result of chaining multiple $y(u'd+j+1,v,p+j+1)\geq y(u'd+j,v,p+j)$ inequalities\\
	The third constraint of $\mathrm{PolyLP'}(n)$ can be obtained as follows: for $u'<p'\leq n$, $v'\leq n$
	\begin{align*}
		y'(u',v',p')&=\frac{1}{d}\sum\limits_{(v,p)\in f(v',p')}y(u'd,v,p)\\
		            &=\frac{1}{d}\sum\limits_{(v,p)\in f(v',p')}y(u'd,v,u'd+1)\\
	...
	\begin{align*}
		y'(u'+1,v',p')&=\frac{1}{d}\sum\limits_{(v,p)\in f(v',p')}y(u'd+d,v,p)\\
					  ...
					  &=y'(u',v',u'+1)
	\end{align*}
	where the first inequality is the consequence of the definition of $y$ and $x$'s non-negativity and the second inequality follows from the fourth constraint of $\mathrm{PolyLP}(n)$\\
	...
	\end{align*}
	where the first inequality is the consequence of the fact that $p\geq v'd\implies y(v'd,u,p)\geq\allowbreak y(v'd-1,u,p)=y(v,d-1,u,v'd)$ and $p<v'd\implies y(v'd,u,p)\geq y(v'd-1,u,v'd)$. The second inequality is the consequence of the third constraint of $\mathrm{PolyLP}(n)$
	Concluding
	\begin{align*}
		     &y'(u',v',u')+y'(v',u',p')\\
