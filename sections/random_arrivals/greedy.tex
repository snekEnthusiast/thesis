...
\begin{quote}
	It can be shown that dropping the term $m_{t+1}$ from the first set of equations does not affect the LP solution by much
\end{quote}
Meaning that $m_{t+1}$ can be dropped without affecting the competitiveness bound by more than $o(1)$, which is supported by the fact that \cite{karp} proved a very similar statement. 
...
\[m_t=\alpha_{n+1-t}=(1-\frac{1}{n})^{t-1}\\v_t=\beta_{n+1-t}=\frac{1}{n}(1-\frac{1}{n})^{t-1}\]
